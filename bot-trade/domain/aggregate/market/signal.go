...
// HasSignal returns true if there is any signal (not no signal).
func (s *TradingSignal) HasSignal() bool {
	return s.Type != NoSignalType
}

// SetTarget sets the target price for the signal.
func (s *TradingSignal) SetTarget(target float64) {
	s.Target = target
}

// SetStopLoss sets the stop loss price for the signal.
func (s *TradingSignal) SetStopLoss(stopLoss float64) {
	s.StopLoss = stopLoss
}

// SignalAnalysisResult is the aggregate root for signal analysis results.
// Similar to AnalysisResult but for trading signals.
type SignalAnalysisResult struct {
	...
	Trendlines       []TrendlineDisplay // Active trendlines for display with pre-calculated data points
}

// NewSignalAnalysisResult creates a new SignalAnalysisResult.
func NewSignalAnalysisResult(
	symbol string,
	signals []TradingSignal,
	processingTimeMs int64,
	...
	return &SignalAnalysisResult{
		...
		PriceHistory:     nil, // Set separately via SetPriceHistory
	}
}

// SetPriceHistory sets the price history data.
func (r *SignalAnalysisResult) SetPriceHistory(priceHistory []*PriceData) {
	r.PriceHistory = priceHistory
}

// WithPriceHistory returns a new SignalAnalysisResult with price history set.
func (r *SignalAnalysisResult) WithPriceHistory(priceHistory []*PriceData) *SignalAnalysisResult {
	r.PriceHistory = priceHistory
	return r
}

// HasConfirmedSignals returns true if there are any confirmed signals.
func (r *SignalAnalysisResult) HasConfirmedSignals() bool {
...
func (r *SignalAnalysisResult) HasWatchingSignals() bool {
	for _, s := range r.Signals {
...
func (r *SignalAnalysisResult) GetConfirmedSignals() []TradingSignal {
...
func (r *SignalAnalysisResult) GetWatchingSignals() []TradingSignal {
...
func (r *SignalAnalysisResult) SetTrendlines(trendlines []TrendlineDisplay) {
