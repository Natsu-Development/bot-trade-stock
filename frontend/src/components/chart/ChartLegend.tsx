      ...
      <div className="flex flex-wrap gap-3 text-xs">
          ...
          <>
            {supportCount > 0 && (
              <div className="flex items-center gap-1.5">
                <div
                  className="w-3 h-0.5 bg-[var(--neon-bull)] rounded"
                  style={{
                    background:
                      'repeating-linear-gradient(90deg, #10b981, #10b981 3px, transparent 3px, transparent 6px)',
                  }}
                />
                <span className="text-[var(--text-muted)]">Support ({supportCount})</span>
              </div>
            )}
            {resistanceCount > 0 && (
              <div className="flex items-center gap-1.5">
                <div
                  className="w-3 h-0.5 bg-[var(--neon-bear)] rounded"
                  style={{
                    background:
                      'repeating-linear-gradient(90deg, #ef4444, #ef4444 3px, transparent 3px, transparent 6px)',
                  }}
                />
                <span className="text-[var(--text-muted)]">Resistance ({resistanceCount})</span>
              </div>
            )}
          </>
        )}
        {signals.length > 0 && showSignals && (
          <div className="flex items-center gap-1.5 ml-2 pl-3 border-l border-[var(--border-primary)]/20">
            ...
            <span className="text-[var(--text-muted)]">{confirmedSignals} Confirmed</span>
            {watchingSignals > 0 && (
              <span className="text-[var(--text-muted)]">· {watchingSignals} Potential</span>
            )}
          </div>
        )}
          ...
          <div className="flex items-center gap-1.5 ml-2 pl-3 border-l border-[var(--border-primary)]/20">
            <div className="flex items-center gap-0.5">
              <div
                ...
                style={{
                  background:
                    'linear-gradient(to top, rgba(16, 185, 129, 0.5) 50%, rgba(239, 68, 68, 0.5) 50%)',
                }}
              />
            </div>
        ...
        {showRsi && rsiData.length > 0 && (
          <div className="flex items-center gap-1.5 ml-2 pl-3 border-l border-[var(--border-primary)]/20">
            <svg
              className="w-3 h-3 text-[var(--neon-purple)]"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
