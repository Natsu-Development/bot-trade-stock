...
Repo-local project context lives under `.context/`. This is the shared source for project facts, conventions, skill-shaped domain guidance, and operational runbooks.

Before editing files, read `.context/README.md`, then load the applicable rules by path:

| Path touched | Rules to read |
| --- | --- |
| `bot-trade/**/*.go` | `.context/rules/backend/architecture.md`, `.context/rules/backend/style.md`, `.context/rules/backend/naming.md`, `.context/rules/backend/error-handling.md`, `.context/rules/backend/concurrency.md`, `.context/rules/backend/patterns.md` |
| `frontend/**/*.{ts,tsx}` | `.context/rules/frontend.md` |
| `**/*_test.go`, `frontend/tests/**/*.spec.ts` | `.context/rules/testing.md` |

Load project context skills when the work matches their domain:

| Work area | Skills to read |
| --- | --- |
| Go implementation, review, linting, concurrency, error handling, testing, or static analysis | `.context/skills/golang-mastery/SKILL.md` |
| Go backend architecture, Clean Architecture, DDD, ports/adapters, `bot-trade` layering | `.context/skills/clean-architecture/SKILL.md` |
| React + TypeScript frontend components, hooks, API client, UI conventions | `.context/skills/frontend-patterns/SKILL.md` |
| Trading logic, RSI divergence, trendlines, RS Rating, alerts, stock metrics | `.context/skills/trading-domain/SKILL.md` |

Load runbooks for matching operational tasks:

| Task | Runbook to read |
| --- | --- |
| Analyze a stock symbol through the API | `.context/runbooks/analyze.md` |
| Deploy the trading bot | `.context/runbooks/deploy.md` |
| Refresh stock metrics cache | `.context/runbooks/refresh-metrics.md` |
| Test API endpoints | `.context/runbooks/test-api.md` |

Use `.context/` for project-specific implementation context. Use OMX plugin/skill surfaces under `.codex/skills` only for orchestration workflows and reusable tool behavior. If both apply, load `.context/` first for project facts, then use the narrowest OMX workflow skill needed to execute or verify the task.
</project_rules>

<code_intelligence>
## Code Intelligence Stack

OMX is the orchestration layer; GitNexus is the repository graph; `gopls` is the Go semantic engine. The routing matrix, default flow, tool boundaries, and pre/post-edit policy live in **[`.context/policies/code-intelligence.md`](.context/policies/code-intelligence.md)** — the single source of truth shared with `CLAUDE.md` and every other runtime. Read it before editing any code in this repository.
</code_intelligence>
...
| Spark (explorer/fast) | `gpt-5.5` | low | Fast triage, explore, lightweight synthesis, and low-latency routing. |
...
| `explore` | `gpt-5.5` | low | Fast codebase search and file/symbol mapping (fast-lane, fast) |
...
For Go changes under `bot-trade`, run `gopls.go_diagnostics` on active changed files when available, then run targeted `go test` checks. Escalate to `go test ./...`, `go vet ./...`, `golangci-lint run ./...`, `govulncheck ./...`, or race tests when the change touches shared services, concurrency, dependencies, or production-critical paths.
...
- For repo behavior, execution flow, cross-file dependency, or API-consumer questions, use GitNexus `query`, `context`, `route_map`, `shape_check`, or `api_impact` before raw search.
- Before editing a function, method, class, API handler, or exported Go symbol, run GitNexus upstream `impact` or `api_impact` and state risk, direct callers, affected flows, and any HIGH / CRITICAL warning.
- For Go work, pair GitNexus with `gopls`: use `go_file_context`, `go_package_api`, `go_symbol_references`, `go_rename_symbol`, and `go_diagnostics` for exact language-server evidence.
- For Go renames, never use raw find-and-replace. Prefer `gopls.go_rename_symbol` for Go-only semantic renames after GitNexus impact; use GitNexus rename when the rename crosses non-Go files, generated wiring, API contracts, or text/config references.
- If a task has independent discovery lanes, split them: one lane maps graph/impact with GitNexus, another checks Go semantics with `gopls`, and the leader reconciles before edits.
...
> **Source of truth:** The block below is auto-generated by `gitnexus ai-context`. The canonical, hand-maintained policy lives at [`.context/policies/code-intelligence.md`](.context/policies/code-intelligence.md). When the two disagree, the policy file wins.
...
- NEVER rename symbols with find-and-replace. Use `gitnexus_rename` for graph-aware or cross-surface renames, or `gopls.go_rename_symbol` for Go-only semantic renames after GitNexus impact.
...
# gopls - Go Language Intelligence

The gopls "Always Do" rules and GitNexus/gopls pairing contract live in **[`.context/policies/code-intelligence.md`](.context/policies/code-intelligence.md)** — read it before any change to `bot-trade/**/*.go`.
